 ...
 * @author Mabeesha
 */
public class AtrialFibrillation_past_data_Learner 
{
    ...
    private final double m;//number of data
    
    private final double convergenceCheckingParameter = 0.001;
    
    public AtrialFibrillation_past_data_Learner(ArrayList<Double>[] pastData,int arraySize)
    {
        ...
        training_data = new ArrayList[training_data_arraySize];  
        for(int i = 0 ; i < training_data_arraySize; i++)
        {
            training_data[i] = new ArrayList<>();
        }
    ...
    private void learn()
    {
        ...
        while(!isConverged)                                                                                     //repeat untill convergence
        {
            ...
            System.arraycopy(temp_parameters, 0, hypothesisParameters, 0, training_data_arraySize-1);
            //System.out.println(hypothesisParameters[0]+" "+hypothesisParameters[1]+" "+hypothesisParameters[2]+" "+hypothesisParameters[3]+" "+hypothesisParameters[4]+" "+hypothesisParameters[5]+" "+hypothesisParameters[6]);
        }
        //System.out.println("past data learning finished, algorithm :- gradient descent");
        //System.out.println("hypothesisParameters;");
        
        for(int i = 0; i < 10; i++) {
        	OutputString += hypothesisParameters[i] + " ";
        }
       // OutputString = hypothesisParameters[0]+" "+hypothesisParameters[1]+" "+hypothesisParameters[2]+" "+hypothesisParameters[3]+" "+hypothesisParameters[4]+" "+hypothesisParameters[5]+" "+hypothesisParameters[6]+" "+hypothesisParameters[7]+" "+hypothesisParameters[8]+" "+hypothesisParameters[9];
        double z = 0;
        for( int k = 0 ; k < training_data_arraySize-1 ; k++ )                                  //calculatingg z
        {
            z = z + hypothesisParameters[k]*training_data[k].get(0);
        }  
        System.out.println("your risk is " + ( 1.0 / (1.0 + Math.exp(-z)) )*100+"% for artial");
